---
title: "Pseudobulk Differential Expression Analysis of Tsai datatset"
author: "Alan Murphy"
...
of Tsai group's cells were Excitatory neurons is worth noting when we inspect 
their DEGs. 
...
Tsai group's analysis, further disparities become clear.

```{r comp2_1}
#look at Tsai group DEG direction and effect size
...
ggplot(data=mathys_degs_match[,.N,by=.(celltype,deg_direction)],
  ...
  ggtitle("Cell type counts of Tsai group's DEGs")+
...
ggplot(data=mathys_degs_match,
  ...
  ggtitle("Cell type Log2 Fold Change of Mathys et al.'s DEGs",
          subtitle = paste0("Median LFC of Tsai group's Down and Up regulated DEGs: ",
...
the Tsai group's analysis across the cell types, is a function of the number of 
...
cor_mathys <- 
  cor.test(comp_mathys$cell_counts,
           comp_mathys$degs,method = "spearman",exact = F)
...
cor_our <- cor.test(comp_our$cell_counts,comp_our$degs,method = "spearman",exact = F)
...
grid.arrange(plot_mathys, plot_our, widths=c(0.4,0.5), ncol=2)
```
